...
import { Coins, Search, Plus, Minus, Loader2, User, Clock, Check, X } from "lucide-react";
...
export function AdminCreditsPanel() {
  ...
  useEffect(() => {
    fetchMembers();
    fetchPendingCredits();
  }, [fetchMembers, fetchPendingCredits]);
              ...
              {pendingCredits.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between p-3 bg-yellow-500/5 rounded-lg border border-yellow-500/20">
                  ...
                  <div className="flex items-center gap-2 ml-3">
                    ...
                    <Button size="sm" variant="outline" className="h-7 px-2 text-green-600 border-green-500/30 hover:bg-green-500/10" disabled={processingTxId === tx.id} onClick={() => handleApproveCredit(tx.id)}>
                      {processingTxId === tx.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                    </Button>
                    <Button size="sm" variant="outline" className="h-7 px-2 text-red-600 border-red-500/30 hover:bg-red-500/10" disabled={processingTxId === tx.id} onClick={() => handleRejectCredit(tx.id)}>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="relative">
            ...
            <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t("admin.searchMembers") || "Search members..."} className="pl-10" />
          </div>
          <div className="space-y-2 max-h-[70vh] overflow-y-auto">
            {filteredMembers.length === 0 ? (
              <p className="text-center text-muted-foreground py-8 text-sm">{t("admin.noMembers") || "No members found"}</p>
            ) : (
        ...
        <div className="lg:col-span-3 space-y-4">
          {!selectedMember ? (
            ...
            <>
              ...
              <AdminMemberProfile member={selectedMember} onUpdated={() => fetchMembers()} />
              ...
              <Card className="mobile-transparent-card">
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input type="number" min="1" value={adjustAmount} onChange={(e) => setAdjustAmount(e.target.value)} placeholder="100" />
                  </div>
                  <div>
                    <Label>{t("admin.description") || "Description"}</Label>
                    <Input value={adjustDescription} onChange={(e) => setAdjustDescription(e.target.value)} placeholder="Reason for adjustment..." />
                  </div>
                  <div className="flex gap-3">
                    <Button onClick={() => handleAdjust(true)} disabled={adjusting || !adjustAmount} className="flex-1">
                      {adjusting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                      {t("admin.addCredit") || "Add Credit"}
                    </Button>
                    <Button onClick={() => handleAdjust(false)} disabled={adjusting || !adjustAmount} variant="destructive" className="flex-1">
              ...
              <Card className="mobile-transparent-card">
                <CardHeader>
                  <CardTitle className="text-base">{t("admin.creditHistory") || "Credit History"}</CardTitle>
                </CardHeader>
                <CardContent>
                  {memberTransactions.length === 0 ? (
                    <p className="text-center text-muted-foreground text-sm py-4">{t("credit.noTransactions") || "No transactions yet"}</p>
                  ) : (
                    <div className="space-y-2">
                        ...
                        <div key={tx.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                          <div>
                            <p className="text-sm font-medium">
                              ...
                              {tx.status === 'pending' && (
                                <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full bg-yellow-500/20 text-yellow-600 dark:text-yellow-400">{t("credit.pending")}</span>
                              )}
                              {tx.status === 'rejected' && (
                                <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-500">{t("credit.rejected")}</span>
                              )}
                            </p>
