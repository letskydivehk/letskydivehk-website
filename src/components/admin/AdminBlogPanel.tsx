...
import { Plus, Edit, Trash2, Eye, EyeOff, Save, Loader2 } from "lucide-react";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if ("id" in editingPost) {
        const { error } = await (supabase as any).from("blog_posts").update(postData).eq("id", editingPost.id);
        if (error) throw error;
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="flex items-center gap-3 pt-6">
              <Switch checked={editingPost.is_published} onCheckedChange={(checked) => setEditingPost({ ...editingPost, is_published: checked })} />
